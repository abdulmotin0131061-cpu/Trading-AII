            ...
            cur.execute("""
                ...
                DECLARE
                    ...
                    symbols TEXT[] := ARRAY[
                        ...
                        'LTC/USD', 'XRP/USD', 'SOL/USD', 'BNB/USD',
                        'ADA/USD', 'DOT/USD', 'DOGE/USD', 'AVAX/USD',
                        'XAU/USD', 'XAG/USD', 'WTI/USD', 'BRENT/USD',
                        'GBP/AUD', 'EUR/AUD', 'AUD/JPY', 'CAD/JPY',
                        'CHF/JPY', 'NZD/JPY', 'GBP/CAD', 'EUR/CAD',
                        'GBP/CHF', 'USD/TRY'
                    ];
                BEGIN
...
def update_stats(win=True):
    try:
        with db_conn() as conn:
            ...
            if win:
                cur.execute(
                    "UPDATE trading_stats SET total_trades = total_trades + 1, wins = wins + 1"
                )
            else:
                cur.execute(
                    "UPDATE trading_stats SET total_trades = total_trades + 1, losses = losses + 1"
                )
            conn.commit()
            cur.close()
    except Exception as e:
...
def get_weights(symbol):
    try:
        with db_conn() as conn:
            ...
            return {
                r['name']: r['weight'] * r['accuracy_factor']
                for r in rows
            }
            ...
            for name, active in indicators_status.items():
                if active:
                    cur.execute("""
                        ...
                        SET accuracy_factor =
                            LEAST(
                                GREATEST(accuracy_factor + %s, 0.5),
                                1.5
                            )
                        WHERE symbol = %s
                        AND name = %s
...
def send_telegram_msg(message):
    ...
    if (
        message == _last_sent["text"]
        and time.time() - _last_sent["ts"] < 60
    ):
        return
...
def calculate_ema_signal(df):
    ...
    if (
        ...
        and prev_row["EMA_FAST"] <= prev_row["EMA_SLOW"]
    ):
        return "BUY"
...
def calculate_indicators(values, symbol="EUR/USD"):
    ...
    volatility_low = bool(
        float(pd.Series(tr).iloc[-1]) < (atr_val * 1.5)
    )

    tp = (
        high_series +
        low_series +
        pd.Series(prices_series)
    ) / 3

    volume_series = pd.Series(df["volume"])

    vwap_series = (
        (tp * volume_series).cumsum()
        /
        volume_series.replace(0, 1).cumsum()
    )

    vwap_up = bool(
        pd.Series(prices_series).iloc[-1]
        >
        vwap_series.iloc[-1]
    )
    ...
    vol_spike = (
        ...
        >
        volume_series.rolling(20).mean().iloc[-1] * 1.5
    )
    ...
    def rsi_signal(rsi_val, side):
        return (
            (side == "BUY" and rsi_val < 40)
            or
            (side == "SELL" and rsi_val > 60)
        )
    ...
    if len(df) >= 3:
        ...
        bullish_pin = (
            lower_wick3 >= 2 * body3
            and upper_wick3 <= body3
            and side_ema == "BUY"
        )

        bearish_pin = (
            upper_wick3 >= 2 * body3
            and lower_wick3 <= body3
            and side_ema == "SELL"
        )
...
def get_batch_market_data(symbols_list, api_key):
    ...
    url = (
        f"https://api.twelvedata.com/time_series?"
        f"symbol={quote(','.join(symbols_list), safe=',')}"
        f"&interval=1min"
        f"&outputsize=75"
        f"&timezone=Asia/Dhaka"
        f"&apikey={api_key.strip()}"
    ...
    try:
        with db_conn() as conn:
            ...
            """, (
                ...
                trade['expiry_time'],
                trade['rec_time'],
                json.dumps(trade['indicators_status']),
                next_retry
            ))
...
def get_candle_at_time(symbol, target_time, api_key):
    ...
    if isinstance(target_time, (int, float)):
        ...
        target_dt = target_time.astimezone(
            BD_TZ
        ).replace(second=0, microsecond=0)
...
def update_expired_trades():
    # ACTIVE থেকে PENDING_RESULT-এ পরিণত করবে যা স্ক্যানারকে তাৎক্ষণিক নতুন সিগন্যাল খুঁজতে দেয়
...
def check_result():
    ...
    for trade in pending_trades:
        ...
        exit_key = os.getenv(
            "RESULT_KEY_EXIT",
            API_KEYS[1] if len(API_KEYS) > 1 else API_KEYS[0]
        )

        entry_price = get_candle_at_time(
            symbol,
            start_time,
            entry_key
        )

        exit_price = get_candle_at_time(
            symbol,
            expiry,
            exit_key
        )
    ...
    print(e)
        ...
        print(
            ...
            f"Verified Result: {symbol} - {result_emoji} | "
...
def fetch_and_analyze_batch(symbols_chunk, api_key):
    ...
    for symbol, values in batch_data.items():
        ...
        status = {
            'trend': bool(
                (ind['trend_up'] and ind['curr_p'] > ind['sma_50']) or
                (not ind['trend_up'] and ind['curr_p'] < ind['sma_50'])
            ),
            'rsi': bool(rsi_signal(ind['rsi'], side_ema)),
            'bb': bool(
                ind['curr_p'] <= ind['lower'] or
                ind['curr_p'] >= ind['upper']
            ),
            'vol': bool(ind['vol_spike']),
            'adx': bool(not ind['adx_low']),
            'ema_crossover': bool(ind['ema_sig'] != "HOLD"),
            'macd': bool(
                (ind['macd_up'] and side_ema == "BUY") or
                (not ind['macd_up'] and side_ema == "SELL")
            ),
            'atr': bool(ind['volatility_low']),
            'vwap': bool(
                (ind['vwap_up'] and side_ema == "BUY") or
                (not ind['vwap_up'] and side_ema == "SELL")
            ),
        ...
        if saved_id:
            ...
            try:
                with db_conn() as conn:
                    ...
                    cur.execute(
                        "UPDATE trades SET msg_sent = TRUE WHERE id = %s",
                        (saved_id,)
                    )
