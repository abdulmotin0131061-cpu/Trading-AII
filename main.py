...
MAX_CANDLE_AGE_MINUTES = max(1, int(os.getenv("MAX_CANDLE_AGE_MINUTES", "3")))

FINAL_CANDIDATE_COUNT = max(1, int(os.getenv("FINAL_CANDIDATE_COUNT", "5")))

RESULT_PROCESSING_TIMEOUT_MINUTES = max(1, int(os.getenv("RESULT_PROCESSING_TIMEOUT_MINUTES", "10")))
...
def init_db():
    ...
    with db_conn() as conn:
        ...
        cur.execute(
            ...
            ALTER TABLE trades ADD COLUMN IF NOT EXISTS signal_price DOUBLE PRECISION;

            ALTER TABLE trades ADD COLUMN IF NOT EXISTS entry_message TEXT;

            ALTER TABLE trades ADD COLUMN IF NOT EXISTS result_message TEXT;
            ...
            -- Filter stage; keeps the candidate lookup index-supported as

            -- the table grows into the millions of rows.
...
def get_market_data(symbol, api_key, outputsize=100):
    ...
    return _extract_values(
        ...
        _request_json(_api_url(symbol, api_key, outputsize=outputsize), API_TIMEOUT),
...
def _sort_values_chronologically(values):
    ...
    return [item for _, item in sorted(rows.items(), key=lambda x: x[0])]
...
def _train_ml_model(symbol, side, now):
    ...
    if len(np.unique(y)) < 2:
        ...
        return {"status": "INSUFFICIENT_CLASS_DIVERSITY", "samples": len(X_list)}
    ...
    with _ml_train_lock:
        ...
        for symbol in SYMBOLS:
            ...
            for side in ("BUY", "SELL"):
                ...
                return  # one model per call keeps retraining bounded
...
def calculate_indicators(values, symbol="EUR/USD"):
    ...
    latest_closed_time = _parse_candle_datetime(closed_values[-1].get("datetime"))
    ...
    candle_age_minutes = (current_time - latest_closed_time).total_seconds() / 60.0

    if candle_age_minutes < 0 or candle_age_minutes > MAX_CANDLE_AGE_MINUTES:
        ...
        with db_conn() as conn:
            ...
            cur.execute(
                ...
                INSERT INTO trades(
                    ...
                    symbol, entry_price, signal_price, side, start_time, expiry_time, rec_time,
                    ...
                    entry_message, result_message, status, next_retry_time
                ...
                (
                    ...
                    trade.get("signal_price", trade.get("entry_price")),
                    ...
                    trade.get("entry_message"),

                    trade.get("result_message"),
...
def recover_stale_result_processing():

    cutoff = get_now() - timedelta(minutes=RESULT_PROCESSING_TIMEOUT_MINUTES)
    ...
    try:
        ...
        with db_conn() as conn:
            ...
            cur.execute(
                ...
                WHERE status='RESULT_PROCESSING'
        ...
        print(f"Stale Result Recovery Error: {exc}")
...
def complete_trade_and_learn(trade_id, symbol, result, entry_price, indicator_opinions):
    ...
    try:
        ...
        with db_conn() as conn:
            ...
            for name, opinion in (indicator_opinions or {}).items():
                ...
                delta = 0.03 if ((result == "WIN" and correct) or (result == "LOSS" and not correct)) else -0.03
...
def store_trade_message(trade_id, message_type, message):

    column = "entry_message" if message_type == "entry" else "result_message"

    if column not in {"entry_message", "result_message"}:
    ...
    try:
        ...
        with db_conn() as conn:
            ...
            cur.execute(f"UPDATE trades SET {column}=%s, updated_at=NOW() WHERE id=%s", (message, trade_id))
        ...
        print(f"Message Store Error: {exc}")
...
def retry_unsent_messages():
    ...
    try:
        ...
        with db_conn() as conn:
            ...
            cur.execute(
                ...
                SELECT id, msg_sent, result_sent, entry_message, result_message

                FROM trades

                WHERE (msg_sent=FALSE AND entry_message IS NOT NULL)

                   OR (result_sent=FALSE AND result_message IS NOT NULL)
        ...
        print(f"Message Retry Query Error: {exc}")
    ...
    for row in rows:
        ...
        if not row.get("msg_sent") and row.get("entry_message"):

            if send_telegram_msg(row["entry_message"]):

                mark_message_sent(row["id"], "entry")

        if not row.get("result_sent") and row.get("result_message"):

            if send_telegram_msg(row["result_message"]):

                mark_message_sent(row["id"], "result")
...
def check_result():
    ...
    while True:
        ...
        if not complete_trade_and_learn(trade_id, symbol, result, entry_price, indicator_opinions):
        ...
        result_emoji = {"WIN": "✅ WIN", "LOSS": "❌ LOSS", "DRAW": "➖ DRAW"}[result]
        ...
        store_trade_message(trade_id, "result", msg)
...
def run_scanner():
    ...
    if not (0 <= signal_now.second <= 15):

        return



    slot = _get_scan_slot(signal_now)

    target_start = signal_now.replace(second=0, microsecond=0) + timedelta(minutes=1)
    ...
    if has_active_trade():
        ...
        print(f"[{signal_now.strftime('%H:%M:%S')}] Active trade in progress. Scanning skipped.")

        return



    print(f"[{signal_now.strftime('%H:%M:%S')}] Scanning markets for independent indicator signals...")
    ...
    if get_now() >= target_start:

        print("Signal deadline missed; no trade created.")
    ...
    candidates = sorted(
        ...
        [x for x in all_results if x["tradable"]],
    ...
    for candidate in candidates:
        ...
        if get_now() >= target_start:
            ...
            print("Signal deadline missed during layered evaluation; no trade created.")

            return



        conf = round(float(candidate["confidence"]), 1)
        ...
        historical = find_similar_historical_setups(
            ...
            features=candidate["setup_features"],
        ...
        ml_result = ml_predict(candidate["symbol"], candidate["side"], candidate["setup_features"])

        final = make_final_decision(candidate["side"], conf, historical, ml_result)



        if final["decision"] == candidate["side"]:

            candidate = dict(candidate)

            candidate["historical"] = historical

            candidate["ml_result"] = ml_result
    ...
    if not evaluated:
        ...
        print("No candidate passed the layered final decision.")

        return



    best = max(evaluated, key=lambda x: x["final"]["combined_confidence"])

    if get_now() >= target_start:

        print("Signal deadline missed before trade creation; no trade created.")
    ...
    rec_time = _choose_expiry(final["combined_confidence"], atr_pct, historical)
    ...
    if send_telegram_msg(entry_message):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            recover_stale_result_processing()
